...
class Tabbed < Cmless
  ...
  class << self
    ...
    def expand_links(html)
      ...
      a_tags = doc.css('a')
      fail NotACatalogLink.new if a_tags.empty? || a_tags.none? { |a| a['href'] && a['href'].match(/:3000\/catalog\?/) }
      a_tags.map do |a|
        fail("Expected catalog url, not #{a['href']}") unless a['href'].match(/:3000\/catalog\?/)
